...
export default function MemeForm(props) {
  ...
  const handleDrop = () => {
    return {
      addedfile: (file) => {
        ...
        fetch("https://api.cloudinary.com/v1_1/dgc3md7zj/image/upload", {
          method: "POST",
          body: formData,
        })
          .then((res) => res.json())
  ...
  const editSubmit = () => {
    fetch(`https://cjr-meme-api.herokuapp.com/meme/${props.id}`, {
      method: "PUT",
      headers: {
  ...
  const handleSubmit = (e) => {
    ...
    switch (!props.id) {
      ...
      default:
        axios
          .post("https://cjr-meme-api.herokuapp.com/meme", {
            text,
            favorite,
            image,
          })
          .then(() => {
            setText(""), setImage(""), setFavorite(false);
